...
$$
Y = XW + \bold{1}_N b \tag{2}
$$
由于NumPy自带广播机制, 因此实现上的形式依然是$(1)$式.
...
$$
\begin{align}
\bold{d}Y &= \bold{d}(XW + \bold{1}_N b) \tag{3.a}\\
&= \bold{d}(XW) + \bold{d}(\bold{1}_N b) \tag{3.b}\\
&= (\bold{d}X)W + X\bold{d}W + (\bold{d1_N})b + \bold{1}_N\bold{d}b \tag{3.c}\\
&= (\bold{d}X)W + X\bold{d}W + \bold{1}_N\bold{d}b \tag{3.d}\\
\end{align}
$$
...
$$
\bold{d}L = \text{tr}((\frac{\partial L}{\partial Y})^\bold{T}\bold{d}Y) \tag{4}
$$
将$\bold{d}Y$替换, 得到:
$$
\begin{align}
\bold{d}L &= \text{tr}\{(\frac{\partial L}{\partial Y})^\bold{T}[(\bold{d}X)W + X\bold{d}W + \bold{1}_N\bold{d}b]\} \tag{5.a} \\
&= \text{tr}((\frac{\partial L}{\partial Y})^\bold{T}(\bold{d}X)W) + \text{tr}((\frac{\partial L}{\partial Y})^\bold{T}X\bold{d}W) + 
\text{tr}((\frac{\partial L}{\partial Y})^\bold{T}\bold{1}_N\bold{d}b) \tag{5.b}
\end{align}
$$
因此, 我们可以得到:
$$
\begin{align}
\frac{\partial L}{\partial X} &= (W(\frac{\partial L}{\partial W})^\bold{T})^\bold{T} = \frac{\partial L}{\partial Y}W^\bold{T} \tag{6.a} \\
\frac{\partial L}{\partial W} &= ((\frac{\partial L}{\partial Y})^\bold{T}X)^\bold{T} = X^T\frac{\partial L}{\partial Y} \tag{6.b} \\
\frac{\partial L}{\partial b} &= ((\frac{\partial L}{\partial Y})^\bold{T}\bold{1}_N)^\bold{T} = \bold{1}_N^\bold{T}\frac{\partial L}{\partial Y} \tag{6.c} \\
\end{align}
$$
...
Sigmoid层的前向传播公式为:
$$
Y = \sigma(X) = \frac{1}{1 + e^{-X}} \tag{7}
$$
其中, $\sigma$是Sigmoid函数. 反向传播的公式为:
$$
\frac{\partial L}{\partial X} = \frac{\partial L}{\partial Y}\odot Y\odot(1 - Y) \tag{8}
$$
在实现上, 可以将前向传播的运算结果$Y$暂时存储起来, 用于反向传播时的计算.
#### Softmax层
Softmax层的前向传播公式为:
$$
Y = \text{softmax}(X) = \frac{e^X}{\sum_{i=1}^N e^{X_i}} \tag{9}
$$
其中, $X\in \mathbb{R}^{n\times D}$是输入, $Y\in \mathbb{R}^{n\times D}$是输出.
反向传播的公式为:
$$
\frac{\partial L}{\partial X} = \frac{\partial L}{\partial Y} \odot Y \odot (1 - Y) \tag{10}
$$
#### CrossEntropyLoss层
CrossEntropyLoss层的前向传播公式为:
$$
L = -\frac{1}{N}\sum_{i=1}^N{\bold{y}_i}\log{\hat{\bold{y}}_i} \tag{11}
$$
其中, $\bold{y}$为标签向量, $\hat{\bold{y}}$为模型预测向量. $N$为向量长度.
反向传播的公式为:
$$
\frac{\partial L}{\partial \hat{\bold{y}}} = -\frac{1}{N}\frac{\bold{y}}{\bold{\hat{y}}} \tag{12}
$$
### 如何运行
